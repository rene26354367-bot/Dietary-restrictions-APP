...
- [ ] 歷史紀錄管理 (進行中)
    - [ ] 補全 `store.tsx` 缺失的 `updateEntry` 與 `removeEntry` 導出
    - [ ] 實作主畫面日期切換器 (Date Picker)
    - [ ] 開發編輯模式 Modal，支援修正過往紀錄份量
- [ ] 智慧營養建議系統 (進行中)
    - [ ] 串接 Gemini API 進行當週趨勢之語義分析
    - [ ] 首頁實作「AI 每日洞察」小卡片
- [ ] OCR UI 體驗優化 (待啟動)
    - [ ] 實作欄位信心標籤 (基於 `metadata.warnings` 標示異常數值)
    - [ ] 建立使用者校正回饋 Log 機制，用於優化解析邏輯

## 長遠待辦清單 (Future Roadmap)
- [ ] **PWA 支援 (P0)**: 實作 Service Worker 支援離線查詢與主畫面安裝。
- [ ] **數據匯出/備份 (P1)**: 支援匯出使用者數據為 JSON 檔案，防止資料遺失。
